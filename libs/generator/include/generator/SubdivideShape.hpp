...
public:
    ...
    public:
        ...
        Edge generate() const {
            ...
            if ( i_ % 2 == 0 )
                return Edge{ { edge_.vertices[0], static_cast<int>( shape_->vertexCache_.size() ) + i_ / 2 } };

            return Edge{ { static_cast<int>( shape_->vertexCache_.size() ) + i_ / 2, edge_.vertices[1] } };
        }
    ...
    class Vertices {
    public:
        bool done() const noexcept { return vertexIndex_ == shape_->vertexCache_.size() && edges_.done(); }

        ShapeVertex generate() const {
            if ( vertexIndex_ < shape_->vertexCache_.size() ) return shape_->vertexCache_[vertexIndex_];
